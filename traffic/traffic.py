...
def load_data(data_dir):
    ...
    # Iterate through the dir (the second variable, subdirectories, is not required)
    for root, _, files in os.walk(data_dir):
        # Iterate through the files inside the directories
        for f in files:
            # Read the image
            image = cv2.imread(os.path.join(root, f))
            image = cv2.resize(image, (IMG_WIDTH, IMG_HEIGHT))

            # Append image and label to the images and labels lists
            images.append(image)
